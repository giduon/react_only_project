function App(){
    ...
    const ClickEvent03 = (n1, n2) => {
        const result = `${n1} 더하기 ${n2}는 ${n1+n2} 입니다.`;
        console.log(result);
    }
    ...
    return(
        <div className="App">
            <h2>클릭 이벤트</h2>    
            {/* 매개 변수가 0개 일 때는 함수의 이름만 명시하면 됩니다.*/}
            {/* 해당 버튼을 클릭했을 때 ClickEnent01 함수가 동작합니다. */}
            ...
            <button onClick={() => ClickEvent03(1, 2)}>매개 변수 2개</button><br />

            {/* 이벤트 객체를 사용하고자 하는 경우 */}
            <button onClick={(event) => ClickEvent04('hello', event)}>이벤트 객체</button><br />
            
            {/* html 문서에서 요소를 구분하고자 할 때 id라는 속성을 사용합니다. */}
